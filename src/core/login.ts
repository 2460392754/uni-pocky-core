 ...
 * @returns {Promise<any>}
 */
export const autoLogin = async function(tokenName = 'token'): Promise<never | void> {
    const token = uni.getStorageSync(tokenName);

    if (token === '') {
        return Promise.reject(
            new Error('login.autoLogin判断为是首次登录(localStorage的token没有值)')
        );
    } else {
...
// 登录
const login = async function(opts: LoginOptions) {
    try {
